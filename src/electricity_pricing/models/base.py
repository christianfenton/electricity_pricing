...
class ForecastModel(ABC):
    ...
    @abstractmethod
    def fit(
        self, endog: pd.Series, exog: Optional[pd.DataFrame] = None
    ) -> "ForecastModel":
        """
        ...
        Args:
            endog: Endogenous variable (target). 
                dtype should be a `pandas.Series` with datetime index.
            exog: Exogenous variables (features). 
                dtype should be a `pandas.DataFrame` with datetime index.
                Default: None.
