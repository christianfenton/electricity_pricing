...
class TestMessyData:
    def test_dst_handling(self, tmp_path):
        ...
        dates = (['2021-03-28'] * 46 +  # Spring DST (46 periods)
                 ['2021-06-15'] * 48 +  # Normal day
                 ['2021-10-31'] * 50)   # Autumn DST (50 periods)

        df_elec = pd.DataFrame({
            'settlementDate': dates,
            'settlementPeriod': list(range(1, 47)) + list(range(1, 49)) + list(range(1, 51)),
            'marketIndexPrice': np.random.rand(len(dates)) * 100,
            'GAS': np.random.rand(len(dates)) * 10000
        })
        ...
        df_forecast = pd.DataFrame({
            'SETTLEMENT_DATE': dates,
            'SETTLEMENT_PERIOD': list(range(1, 47)) + list(range(1, 49)) + list(range(1, 51)),
            'DEMAND_FORECAST': np.random.rand(len(dates)) * 30000
        })
        ...
        spring_dst_data = df_merged[df_merged['SETTLEMENT_DATE'] == pd.Timestamp('2021-03-28')]
        autumn_dst_data = df_merged[df_merged['SETTLEMENT_DATE'] == pd.Timestamp('2021-10-31')]
        normal_day_data = df_merged[df_merged['SETTLEMENT_DATE'] == pd.Timestamp('2021-06-15')]
    ...
    def test_missing_and_duplicate_handling(self, tmp_path):
        ...
        dates_elec = ['2021-01-01'] * 46 + ['2021-01-02'] * 49  # missing and extra periods
        periods_elec = list(range(1, 47)) + [1, 1] + list(range(2, 49))  # duplicates
        ...
        for day_date in ['2021-01-01', '2021-01-02']:
            day_data = df_merged[df_merged['SETTLEMENT_DATE'] == pd.Timestamp(day_date)]
            assert len(day_data) == 48, f"Day {day_date} should have 48 periods"
