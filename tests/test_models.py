...
class TestARXModel:
    ...
    def test_fit_no_exog(self):
        ...
        n_obs = 10
        dates = pd.date_range('2024-01-01', periods=n_obs, freq='30min', tz='Europe/London')
        endog_obs = pd.Series(np.arange(1, n_obs+1), index=dates)
        
        # Create AR model
        lags = [1, 2]
        max_lag = np.max(lags)
        model = ARXModel(lags=lags, regressor=LinearRegression(fit_intercept=True))
        model.fit(endog_obs, exog=None)
        coef, intercept = model.get_params()

        # Manually construct autoregressive terms and fit regressor
        features = pd.DataFrame({f'lag_{lag}': endog_obs.shift(lag) for lag in lags}).dropna()
    ...
    def test_fit_with_exog(self):
        ...
        features = pd.concat([features.reset_index(drop=True), exog_aligned], axis=1)
        regressor = LinearRegression(fit_intercept=True)
        regressor.fit(features.values, targets.values)
        coef_expected = regressor.coef_
        intercept_expected = regressor.intercept_
    ...
    def test_forecast_with_exog(self):
        ...
        dates_future = pd.date_range(
            dates[-1] + pd.Timedelta('30min'), periods=1, freq='30min', tz='Europe/London')
        exog_future = pd.DataFrame({'feature1': [0.5],'feature2': [0.6]}, index=dates_future)
        ...
        dates_future_multi = pd.date_range(
            dates[-1] + pd.Timedelta('30min'), periods=n_steps, freq='30min', tz='Europe/London')
        exog_future_multi = pd.DataFrame({
            'feature1': np.random.default_rng(seed=3).uniform(size=n_steps),
            'feature2': np.random.default_rng(seed=4).uniform(size=n_steps)
        ...
        buffer = history.copy()
        for i in range(n_steps):
            features = np.concatenate([buffer.values, exog_future_multi.iloc[i].values])
        ...
        np.testing.assert_allclose(forecast_multi, forecasts_expected, rtol=1e-10)